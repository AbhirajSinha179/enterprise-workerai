...
export function EmailList({ targetId }: EmailListProps) {
  ...
  const fetchUnscheduledEmails = async (
    targetId: string,
    limit: number,
    skip: number
  ): Promise<UnscheduledEmailThread[]> => {
    ...
    <main className="flex w-full flex-col items-center">
      <div className="mt-6 flex w-full items-center justify-between">
        ...
        <div className="flex flex-row space-x-4">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center">
                    <Switch disabled id="autopilot-mode" className="mx-2" checked={true} />
          ...
          <Button onClick={() => handleApproveAll(userId)}>Approve and Smart Schedule All</Button>
        </div>
      </div>
      ...
      <PaginationWithLinks page={currentPage} pageSize={pageSize} onPageChange={handlePageChange} />
    </main>
  )
}
